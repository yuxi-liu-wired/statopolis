...
public class Board extends Application {
    ...
    private void goAllTheWayBackwards() {
    ...
    private boolean canGoForwards() {
        return game.getTurnNumber() != gameHistory.size();
    }
    ...
    private void goAllTheWayForwards() {
        game = gameHistory.get(gameHistory.size() - 1);
    ...
    // This method highlights the move. Makes it easier to see.
    private void makeHighlight() {
        ...
        Coordinate coord = new Coordinate("ABCDEFGHIJKLMNOPQRSTUVWXYZ".indexOf(moveString.charAt(0)), "ABCDEFGHIJKLMNOPQRSTUVWXYZ".indexOf(moveString.charAt(1)));
        Move m = new Move(coord, moveString.substring(2, 3), moveString.substring(3, 4));
    ...
    private void makeHelpScreen() {
        ...
        helpRectangle.setHeight(20 * SQUARE_SIZE);
        ...
        Text helpText = new Text("  To make a move as a human, click and drag to move the pieces, and use scroll wheel to rotate the pieces.\n  To make the computer opponent make a move, click the \"Move\" button.\n  Click \"New Game\" to start a new game.\n  Click \"Save Game\" to generate a save code for the current game and get it into the system's clipboard.\n  Paste the save code generated by the \"Save Game\" button into the text box below and click \"Load Game\" to load a previously saved game.\n  Click the arrow buttons to browse game history. The single arrows buttons (or press <ctrl>+<arrow> on keyboard) go one step, while the double arrow buttons (<ctrl>+<shift>+<arrow>) go all the way to the end.\n  Warning: if you make a move at a historical location, history will be rewritten and you can't return to the future.\n  To learn to play Stratopolis, please search on Google.");
        ...
        quitHelpButton.setLayoutY((UP_MARGIN + 22) * SQUARE_SIZE);
    ...
    private void makeControls() {
        ...
        hb.setSpacing(10);
        hb.setLayoutX(LEFT_MARGIN * SQUARE_SIZE);
        ...
        forwardsButton.setLayoutX(4 * SQUARE_SIZE);
        ...
        Button allTheWayForwardsButton = new Button(">>");
        allTheWayForwardsButton.setLayoutX(4 * SQUARE_SIZE);
        allTheWayForwardsButton.setLayoutY((UP_MARGIN + 26) * SQUARE_SIZE);
        allTheWayForwardsButton.setPrefWidth(2 * SQUARE_SIZE);
        allTheWayForwardsButton.setOnAction(new EventHandler<ActionEvent>() {
            @Override public void handle(ActionEvent e) {
                goAllTheWayForwards();
            }
        });
        controls.getChildren().add(allTheWayForwardsButton);

        Button backwardsButton = new Button("<");
        backwardsButton.setLayoutX(1 * SQUARE_SIZE);
        ...
        Button allTheWayBackwardsButton = new Button("<<");
        allTheWayBackwardsButton.setLayoutX(1 * SQUARE_SIZE);
        allTheWayBackwardsButton.setLayoutY((UP_MARGIN + 26) * SQUARE_SIZE);
        allTheWayBackwardsButton.setPrefWidth(2 * SQUARE_SIZE);
        allTheWayBackwardsButton.setOnAction(new EventHandler<ActionEvent>() {
            @Override public void handle(ActionEvent e) {
                goAllTheWayBackwards();
            }
        });
        controls.getChildren().add(allTheWayBackwardsButton);

        if(!canGoBackwards()) {
            backwardsButton.setDisable(true);
            allTheWayBackwardsButton.setDisable(true);
        }
        if(!canGoForwards()) {
            forwardsButton.setDisable(true);
            allTheWayForwardsButton.setDisable(true);
        }
        ...
        EventHandler<KeyEvent> keyListener = new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent event) {
                ...
                if (code == KeyCode.RIGHT && event.isControlDown() && event.isShiftDown() && canGoForwards()) {
                    goAllTheWayForwards();
                } else if (code == KeyCode.LEFT && event.isControlDown() && event.isShiftDown() && canGoBackwards()) {
                    goAllTheWayBackwards();
                } else if (code == KeyCode.RIGHT && event.isControlDown() && canGoForwards()) {
                    goForwards();
                } else if (code == KeyCode.LEFT && event.isControlDown() && canGoBackwards()) {
                    goBackwards();
                }
                event.consume();
            }
        };
        controls.setOnKeyReleased(keyListener);
    ...
    // Generates a clean history up to the current game state.
    private void rewriteHistory() {
        gameHistory.clear();
        Game historicalGame = game.clone();
        gameHistory.add(0,historicalGame);
        while (historicalGame.canTakeBackAMove()) {
            historicalGame = historicalGame.returnPreviousGame();
            gameHistory.add(0,historicalGame);
        }
    }
